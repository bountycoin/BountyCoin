...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "importprivkey <BountyCoinPrivkey> [label]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <BountyCoinAddress>\n"
            "Reveals the private key corresponding to <BountyCoinaddress>.");
    ...
    return CBountyCoinSecret(vchSecret, fCompressed).ToString();
}
